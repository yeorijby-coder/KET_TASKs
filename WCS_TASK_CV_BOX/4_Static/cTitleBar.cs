...
namespace WCS_TASK_CV
{
    /*
     * 폼 타이틀바 표시 클래스
     *   - 시스템 구성정보(PLC / DB / DB TABLE / COMM)를 타이틀에 표시한다.
    ...
    public class cTitleBar
    {
        ...
        {
#if ORACLE
            return "[DB(Oracle) : " + cDefApp.GM_DB1_ALIAS + "@" + cDefApp.GM_DB1_USERID + "]";
#elif POSTGRESQL
            return "[DB(PostgreSQL) : " + cDefApp.GM_DB2_DATABASE + "@" + cDefApp.GM_DB2_USER
                                        + "/" + cDefApp.GM_DB2_IP + ":" + cDefApp.GM_DB2_PORT + "]";
#elif SQL
            return "[DB(MS-SQL) : " + cDefApp.GM_DB2_DATABASE + "@" + cDefApp.GM_DB2_USER
                                    + "/" + cDefApp.GM_DB2_IP + ":" + cDefApp.GM_DB2_PORT + "]";
#else
            return "[DB(None)]";
#endif
