...
namespace TSK_COMM_IOSCH
{
    ...
    public class cThread_SCH_1F : IOSchDB
    {
        ...
        private const string HSPOS_STO_1F = "1";   // RANK_1 (D176 입고 출발 H/S)
        private const string HSPOS_RET_1F = "2";   // RANK_2 (D180 출고 도착 H/S)
        ...
        { return ScStoreRoutine(strWH_TYP, CV_PLC_1F_OLD, HSPOS_STO_1F, HSPOS_RET_1F, "[StoHsCheck2]", ref pRTN_MSG); }

        public bool StoHsCheck5(string strWH_TYP, string strPLC_NO, ref string pRTN_MSG)
        { return ScStoreRoutine(strWH_TYP, CV_PLC_1F_NEW, HSPOS_STO_1F, HSPOS_RET_1F, "[StoHsCheck5]", ref pRTN_MSG); }
        ...
        private bool ScRetrieveRoutine(string strWH_TYP, string strHS_NO, string strTitle, ref string pRTN_MSG)
        {
            try
            {
                ...
                if (UpdateScData(strSC_NO, JT_RET, strLUGG_NO, strSTART_LOC, "",
                                 HSPOS_STO_1F, HSPOS_RET_1F, ref pRTN_MSG) == false)
                {
                    _pBdb.Rollback();
                    pRTN_MSG = "";
