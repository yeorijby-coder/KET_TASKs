...
namespace TSK_COMM_IOSCH
{
    ...
    public class cThread_SCH : IOSchDB
    {
         ...
         * 창고 구분.
         *
         *   원래 KET_WH02(=20) 로 두고 "여기는 02 임!" 이라 적혀 있었으나,
         *   이 현장 데이터는 BOX 라인 것도 전부 10 이다.
         *     - CV_DATA  : PLC 04 의 BOX 트랙(410/411/419/420) 전부 WH_TYP='10'
         *     - SC_DATA  : BOX 크레인 911호기 WH_TYP='10'
         *     - SC_HS_DEF: 911 / HS_NO='02' / HS_MC_NO=410  WH_TYP='10'
         *     - 작업을 넣는 WCS_TASK_HOST 의 modDefApp.WH_TYP 도 "10" 고정이다.
         *   20 으로 두면 어느 조회도 한 건도 못 잡아 BOX 스레드가 아무 일도 하지 않는다.
         *   (창고를 실제로 둘로 나누게 되면 이 값과 위 데이터를 같이 바꿔야 한다)
         */
        ...
        public void Thread_Doing(object value)
        {
            ...
            while (!m_bManualStop)
            {
                ...
                try
                {
                    ...
                    // ── 크레인(SC) : 레거시 CSc::StoreRoutine(RANK_1) / RetrieveRoutine(RANK_2)
                    RunSchFunc(StoHsCheck4);            // BOX 입고 H/S -> 크레인 입고 지시
                    RunSchFunc(RetCmdCheck4);           // BOX 출고 지시 20 -> 21
                    RunSchFunc(ScCompleteCheck4);       // 크레인 작업 완료 -> 29
        ...
        // ─────────────────────────────────────────────────────────────────
        // BOX 라인 크레인 지시 - 레거시 ECS CSc::StoreRoutine / RetrieveRoutine (SC_1)
        //
        //   BOX 라인 크레인은 SC 11호기 하나다. (레거시 SC_1 = To-Be 911호기)
        //   PLT 크레인과 달리 층을 두 개 보지 않는다. 레거시 CLib::GetScHSTrackPerRank
        //   (Lib.cpp:534) 에서 SC_1 은 RANK_1 / RANK_2 두 자리만 쓴다.
        //
        //     RANK_1 = 입고 H/S  트랙 4020(포크1) / 4019(포크2)  -> To-Be 420 / 419
        //     RANK_2 = 출고 H/S  트랙 4010(포크1) / 4011(포크2)  -> To-Be 410 / 411
        //
        //   그래서 SC_DATA 에 실을 H/S 자리 번호는 1 과 2 이고, SC_HS_DEF.HS_NO 도 '02' 다.
        //   (3층 PLT 라인은 3 과 4, '04' 를 쓴다 - cThread_SCH_3F)
        //
        //   BOX 로 나갈 출고 작업은 도착 작업대로 가른다. 레거시 CLib::GetRank
        //   (Lib.cpp:2343) 가 RANK_2 를 돌려주는 3층 BOX 작업대가 STN_3F_BOX 다.
        //
        //   ※ 레거시 SC_1 은 Twin 포크다. 여기서는 1층/3층 PLT 판과 같이 포크1만 쓴다
        //     (USE_FK_OD='0'). 포크2 동시 지시(레거시 FetchScRetJobByScNumForTwinFork)는
        //     아직 넣지 않았다. 넣으려면 SC_DATA 의 _FK2_OD 자리를 같이 채우고
        //     USE_FK_OD 를 '1' 로 주면 된다.
        // ─────────────────────────────────────────────────────────────────
        #region BOX 라인 크레인 지시

        // SC_HS_DEF.HS_NO : BOX 출고 H/S (레거시 SC_1 의 RANK_2)
        private const string HS_NO_RET_BOX = "02";

        // SC_DATA 에 실을 H/S 자리 번호 = 레거시 RANK 값 그대로
        private const string HSPOS_STO_BOX = "1";   // RANK_1 (D176 입고 출발 H/S)
        private const string HSPOS_RET_BOX = "2";   // RANK_2 (D180 출고 도착 H/S)

        // 3층 BOX 출고 작업대 - 레거시 CLib::GetRank 가 RANK_2 를 돌려주는 자리
        private static readonly string[] STN_3F_BOX =
            { "211", "221", "222", "231", "241", "242", "251" };

        // ── ECS *4 대응 크레인 공개 함수 ──
        public bool StoHsCheck4(string strWH_TYP, string strPLC_NO, ref string pRTN_MSG)
        { return ScStoreRoutine(strWH_TYP, CV_PLC_3F_BOX, "[StoHsCheck4]", ref pRTN_MSG); }
        public bool RetCmdCheck4(string strWH_TYP, string strPLC_NO, ref string pRTN_MSG)
        { return ScRetrieveRoutine(strWH_TYP, HS_NO_RET_BOX, "[RetCmdCheck4]", ref pRTN_MSG); }
        public bool ScCompleteCheck4(string strWH_TYP, string strPLC_NO, ref string pRTN_MSG)
        { return SC_COMP_CHK(strWH_TYP, "[ScCompleteCheck4]", ref pRTN_MSG); }

        // ─────────────────────────────────────────────────────────────────
        // BOX 입고 H/S 도착 -> 크레인 입고 지시 (레거시 CSc::StoreRoutine(RANK_1) + Store)
        ...
        //   1층 판과 다른 것은 CV PLC(04)와 대상 크레인뿐이다. (H/S 자리 번호는 같다)
        // ─────────────────────────────────────────────────────────────────
        private bool ScStoreRoutine(string strWH_TYP, string strCV_PLC, string strTitle, ref string pRTN_MSG)
        {
            try
            {
                ...
                if (UpdateScData(strSC_NO, JT_STO, strLUGG_NO, "", strDEST_LOC,
                                 HSPOS_STO_BOX, HSPOS_RET_BOX, ref pRTN_MSG) == false)
                {
                    _pBdb.Rollback();
                    pRTN_MSG = "";
                ...
                pRTN_MSG = strTitle + "TRACK " + strMC_NO + "번[BOX 입고 H/S]에서 SC_TASK를 통해서 "
        ...
        // ─────────────────────────────────────────────────────────────────
        // BOX 출고 지시 (20 -> 21) - 레거시 CSc::RetrieveRoutine(RANK_2) + Retrieve
        //   상위가 넣은 출고 작업 중 BOX 로 나갈 것만 골라 크레인에 넘긴다.
        //     꺼낼 곳     : JM.START_LOCATION (랙 위치)
        //     내려놓을 곳 : 그 호기의 BOX 출고 H/S (SC_HS_DEF.HS_NO='04')
        ...
        private bool ScRetrieveRoutine(string strWH_TYP, string strHS_NO, string strTitle, ref string pRTN_MSG)
        {
            try
            {
                ...
                // ── 1) BOX 로 나갈 출고 작업만 고른다 (호기는 아직 모른다)
                //      레거시 CLib::GetRank 가 RANK_2 를 주는 BOX 작업대
                strSql = "";
                strSql += CRLF + " SELECT LUGG_NO, JOB_TYP, START_POS, START_LOCATION, DEST_POS ";
                strSql += CRLF + "   FROM JOB_MST                                            ";
                strSql += CRLF + "  WHERE WH_TYP           = :WH_TYP                         ";
                strSql += CRLF + "    AND JOB_STATUS       = '" + ST_SC_WAIT + "'            ";   // 20 = SC 구동요구
                strSql += CRLF + "    AND DEST_POS IN (" + SqlInList(STN_3F_BOX) + ")       ";
                ...
                for (int i = 0; i < dtJob.Rows.Count; i++)
                {
                    ...
                    // ── 4) 그 호기와 BOX 출고 H/S 가 지시를 받을 수 있는 상태인지 본다
                    strSql = "";
                    ...
                    strSql += CRLF + "    AND SHD.HS_NO           = :HS_NO                      ";   // BOX 출고 H/S
                    strSql += CRLF + "    AND SHD.HS_USE_YN       = 'Y'                         ";
                    strSql += CRLF + "  INNER JOIN CV_DATA CD                                   ";
                ...
                // 이어받을 CV_RETHS_PLC 가 HS_TRACK_NO 로 BOX 출고 H/S 트랙을 찾는다
                ...
                pRTN_MSG = strTitle + strSC_NO + "호기에 BOX 출고를 지시하였습니다. [작업번호:" + strLUGG_NO
        ...
        // ─────────────────────────────────────────────────────────────────
        // 크레인 작업 완료 -> 작업 29(SC 구동완료 보고) - 레거시 CSc::CompleteCheck
        //   COMPLETE_RD(레거시 D110)가 0 이 아니면 완료다. (1=포크1, 2=포크2, 3=전체)
        ...
        //   출고는 29 이후 CV 구간(11 -> 19)이 남는다.
        //   완료를 확인했으면 포크 데이터를 지워 크레인을 다음 작업에 쓸 수 있게 한다.
        //
        //   ※ 이 함수는 라인을 가리지 않는다. 크레인이 여러 자리를 같이 보므로
        //      다른 스레드의 같은 함수와 겹칠 수 있다. 먼저 잡은 쪽이 상태를 29 로
        //      바꾸므로 나중 쪽은 WHERE JOB_STATUS='21' 에서 걸러져 아무것도 안 한다.
        // ─────────────────────────────────────────────────────────────────
        ...
        /*
         * SqlInList :: 문자열 배열을 IN 절 목록으로 만든다. ('200','201',...)
            ...
            )
        {
            try
            {
                ...
                //   BOX 크레인은 레거시상 Twin 이지만 아직 포크1만 쓴다.
